...
import { icons, images, COLORS, FONTS, SIZES } from './index'
...
const ScrollableCard = ({ navigation, productList }) => {
    ...
    const renderCard = ({ item }) => (
        <TouchableOpacity
            ...
            onPress={() => navigation.navigate("FurnitureDetail", { "itemInfo": item })}
        >
...
const FurnitureApp = (props) => {
    ...
    function renderTitle(title) {
        return (
            <View style={{ marginTop: SIZES.font, marginHorizontal: SIZES.padding }}>
                ...
                <Text style={{ color: COLORS.black, ...FONTS.largeTitle, textTransform: 'capitalize' }}>{title}</Text>
            </View>
        )
    }
    ...
    return (
        <>
            <StatusBar backgroundColor={dark ? colors.primary : '#fff'} barStyle={dark ? 'light-content' : 'dark-content'} />
            <View style={[styles.flex1, styles.container, {paddingTop: 10}]}>
                {/* <Text style={{color: colors.text}}>Furniture App</Text> */}
                {renderHeader()}
...
export default React.memo(FurnitureApp)